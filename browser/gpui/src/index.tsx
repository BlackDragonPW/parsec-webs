...
interface NeutronConfig {
  fontSize:    number
  /** Override font path (optional — Rust falls back to system font) */
  fontPath?:   string
}
 ...
 */
export async function initNeutron(config: NeutronConfig): Promise<void> {
...
const SceneContext = createContext<SceneBuffer | null>(null)
 ...
 */
export function NeutronSurface({
  children,
  style,
}: {
  ...
  const rootRef      = useRef<ReturnType<typeof createNeutronRoot> | null>(null)
  const scene        = getScene()
...
// These are the JSX elements Neutron components compose with.
// They map 1:1 to scene buffer primitives.
// Declare them globally so TSX doesn't complain about unknown element types.

declare global {
  namespace JSX {
    interface IntrinsicElements {
      'n-rect':   NRectProps
      'n-border': NBorderProps
      'n-text':   NTextProps
      'n-shadow': NShadowProps
      'n-clip':   NClipProps
    }
  }
}

interface NRectProps {
  x?: number; y?: number; width?: number; height?: number
  color?: number; radius?: number; children?: React.ReactNode
}
interface NBorderProps extends NRectProps { borderWidth?: number }
interface NTextProps extends NRectProps { text: string; fontSize?: number }
interface NShadowProps extends NRectProps {
  blur?: number; spread?: number; offsetX?: number; offsetY?: number
}
interface NClipProps extends NRectProps {}

/** Filled rectangle — the base building block */
export function Rect(props: NRectProps) {
  return <n-rect {...props} />
}

/** Rectangle outline */
export function Border(props: NBorderProps) {
  return <n-border {...props} />
}

/** Text run — renders via the GPU glyph atlas, zero DOM involvement */
export function Text(props: NTextProps) {
  return <n-text {...props} />
}

/** Box shadow */
export function Shadow(props: NShadowProps) {
  return <n-shadow {...props} />
}

/** Clip region — clips all children to the given rect */
export function Clip(props: NClipProps) {
  return <n-clip {...props} />
}
...
export function useNeutronScene(): SceneBuffer | null {
...
export type { SceneBuffer } from './scene'
