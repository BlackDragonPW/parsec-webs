...
export class SceneBuffer {
     ...
     */
    border(x, y, w, h, color, width, radius = 0) {
        if (this.full)
            return;
        const base = this.cursor;
     ...
     */
    textRun(x, y, color, fontSize, glyphIds) {
        if (this.full)
            return;
     ...
     */
    shadow(x, y, w, h, color, blur, spread = 0, offX = 0, offY = 0) {
        if (this.full)
            return;
        const base = this.cursor;
