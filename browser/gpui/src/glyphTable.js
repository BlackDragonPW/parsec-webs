...
let glyphIds = new Uint16Array(TABLE_SIZE);
 ...
 */
export async function initGlyphTable(fontSizePx) {
    ...
    const result = await invoke('neutron_init_glyph_table', {
 ...
 */
export function glyphId(codepoint) {
 ...
 */
export function stringToGlyphIds(str) {
 ...
 */
export function stringToGlyphIdsInto(str, out, offset = 0) {
...
function schedulePendingRasterize(cp) {
    pending.add(cp);
    if (!flushScheduled) {
        flushScheduled = true;
...
async function flushPending() {
    ...
    pending.clear();
    try {
        const result = await invoke('neutron_rasterize_glyphs', { codepoints: cps });
        for (let i = 0; i < result.length; i += 2) {
            const cp = result[i];
            const id = result[i + 1];
            if (cp < TABLE_SIZE)
